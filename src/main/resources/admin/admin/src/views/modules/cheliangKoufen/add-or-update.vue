...
-->
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="车辆扣分编号" prop="cheliangKoufenUuidNumber">
                       <el-input v-model="ruleForm.cheliangKoufenUuidNumber"
                                 placeholder="车辆扣分编号" clearable  :readonly="ro.cheliangKoufenUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="车辆扣分编号" prop="cheliangKoufenUuidNumber">
                           <el-input v-model="ruleForm.cheliangKoufenUuidNumber"
                                     placeholder="车辆扣分编号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="扣分类型" prop="cheliangKoufenTypes">
                        <el-select v-model="ruleForm.cheliangKoufenTypes" :disabled="ro.cheliangKoufenTypes" placeholder="请选择扣分类型">
                            <el-option
                                v-for="(item,index) in cheliangKoufenTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="扣分类型" prop="cheliangKoufenValue">
                        <el-input v-model="ruleForm.cheliangKoufenValue"
                            placeholder="扣分类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.cheliangKoufenContent"  label="扣分缘由" prop="cheliangKoufenContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.cheliangKoufenContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.cheliangKoufenContent" label="扣分缘由" prop="cheliangKoufenContent">
                            <span v-html="ruleForm.cheliangKoufenContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type!='info' && !ro.koufenTime"  class="input" label="扣分时间" prop="koufenTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.koufenTime"
                                type="datetime"
                                placeholder="扣分时间"
                                :disabled="ro.koufenTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.koufenTime" label="扣分时间" prop="koufenTime">
                            <span v-html="ruleForm.koufenTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="分值" prop="cheliangKoufenNumber">
                       <el-input v-model="ruleForm.cheliangKoufenNumber"
                                 placeholder="分值" clearable  :readonly="ro.cheliangKoufenNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="分值" prop="cheliangKoufenNumber">
                           <el-input v-model="ruleForm.cheliangKoufenNumber"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    cheliangId: false,
                    yonghuId: false,
                    cheliangKoufenUuidNumber: false,
                    cheliangKoufenTypes: false,
                    cheliangKoufenContent: false,
                    koufenTime: false,
                    cheliangKoufenNumber: false,
                    insertTime: false,
                },
                ruleForm: {
                    cheliangId: '',
                    yonghuId: '',
                    cheliangKoufenUuidNumber: new Date().getTime(),
                    cheliangKoufenTypes: '',
                    cheliangKoufenContent: '',
                    koufenTime: '',
                    cheliangKoufenNumber: '',
                    insertTime: '',
                },
                cheliangKoufenTypesOptions : [],
                cheliangOptions : [],
                yonghuOptions : [],
                rules: {
                   cheliangId: [
                   ...
                   cheliangKoufenUuidNumber: [
                              { required: true, message: '车辆扣分编号不能为空', trigger: 'blur' },
                          ],
                   cheliangKoufenTypes: [
                              { required: true, message: '扣分类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   cheliangKoufenContent: [
                              { required: true, message: '扣分缘由不能为空', trigger: 'blur' },
                          ],
                   koufenTime: [
                              { required: true, message: '扣分时间不能为空', trigger: 'blur' },
                          ],
                   cheliangKoufenNumber: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=cheliang_koufen_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.cheliangKoufenTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                ...
                _this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        _this.ruleForm.cheliangKoufenContent = _this.ruleForm.cheliangKoufenContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.cheliangKoufenContent = this.ruleForm.cheliangKoufenContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`cheliangKoufen/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.cheliangKoufenCrossAddOrUpdateFlag = false;
                                        this.parent.search();
