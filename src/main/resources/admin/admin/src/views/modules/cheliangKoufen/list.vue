        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '扣分类型' : ''">
                        <el-select v-model="searchForm.cheliangKoufenTypes" placeholder="请选择扣分类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in cheliangKoufenTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('cheliangKoufen','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/cheliangweizhang/upload/cheliangKoufenMuBan.xls"
                        >批量导入车辆扣分数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('cheliangKoufen','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="cheliangKoufenUploadSuccess"
                                :on-error="cheliangKoufenUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('cheliangKoufen','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入车辆扣分数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('cheliangKoufen','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "cheliangKoufen.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="cheliangKoufenUuidNumber"
                                   header-align="center"
                                   label="车辆扣分编号">
                        <template slot-scope="scope">
                            {{scope.row.cheliangKoufenUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="cheliangKoufenTypes"
                                      header-align="center"
                                      label="扣分类型">
                        <template slot-scope="scope">
                            {{scope.row.cheliangKoufenValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="扣分时间">
                        <template slot-scope="scope">
                            {{scope.row.koufenTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="cheliangKoufenNumber"
                                      header-align="center"
                                      label="分值">
                        <template slot-scope="scope">
                            {{scope.row.cheliangKoufenNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('cheliangKoufen','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('cheliangKoufen','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('cheliangKoufen','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              cheliangKoufenTypesSelectSearch : [],
            form:{
                id : null,
                cheliangId : null,
                yonghuId : null,
                cheliangKoufenUuidNumber : null,
                cheliangKoufenTypes : null,
                cheliangKoufenContent : null,
                koufenTime : null,
                cheliangKoufenNumber : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '车辆扣分编号': "cheliangKoufenUuidNumber",
                     '扣分类型': "cheliangKoufenValue",
                     '扣分时间': "koufenTime",
                     '分值': "cheliangKoufenNumber",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                this.$nextTick(()=>{
                    ...
                    let params = {
                        ...
                        groupColumn: "cheliang_koufen_types",
                    }
                    this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.cheliangKoufenTypes!= '' && this.searchForm.cheliangKoufenTypes!= undefined) {
                    params['cheliangKoufenTypes'] = this.searchForm.cheliangKoufenTypes
                }
                                        
                params['cheliangKoufenDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "cheliangKoufen/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=cheliang_koufen_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.cheliangKoufenTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            cheliangKoufenUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "cheliangKoufen/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            cheliangKoufenUploadError(data){
